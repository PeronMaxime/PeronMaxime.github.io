...
Balle.prototype.update = function(zombie, compteurZ, bonus){
  ...
  for(var j=0; j<bonus.length; j++){
    if((balle.x >= bonus[j].destinationX && balle.x <= bonus[j].destinationX+64 && balle.y >= bonus[j].destinationY && balle.y <= bonus[j].destinationY+64) && bonus[j].debloque){
      balle.animation = false;
      bonus.splice(j, 1, '');
    }
  }
